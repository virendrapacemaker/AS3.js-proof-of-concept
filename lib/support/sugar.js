...
_.mixin({
  ...
  enhanceWithGettersAndSetters: function(c) {
    ...
    _.each(c, function(prop, n, obj) {

      // trace('Processing property ' + n);

      if(n.indexOf('get') === 0) {
        n = _.lowercaseFirstLetter(n.substr(3));
  ...
  enhanceWithGettersAndSetters2: function(c) {
    ...
    for(prop in c) {
      ...
      if(prop.indexOf('get') === 0) {
        ...
        if(!_.functionTakesArgs(c[prop])) {
          n = _.lowercaseFirstLetter(prop.substr(3));
          //trace('Name of getter is ' + n);
          props[n] = props[n] || {};
        ...
        if(!_.functionTakesMultipleArgs(c[prop])) {
          n = _.lowercaseFirstLetter(prop.substr(3));
          //trace('Name of setter is ' + n);
          props[n] = props[n] || {};
  ...
  functionTakesArgs: function(func) {
    return func.toString().match(/function(( *|[\w]*)\()(.+)(\))/);
  },

  functionTakesMultipleArgs: function(func) {
    return func.toString().match(/function(( *|[\w]*)\()(.+?,.+)(\))/);
  },
